...
function gcd(...numbers) {
  ...
  for (let i = 1; i < numbers.length; i++)
    result = gcdSimple(result, numbers[i]);
  return result;
}

function lcmSimple(a, b) {
  if (a === 0 && b === 0) return 0;
  return Math.abs(a) * (Math.abs(b) / gcdSimple(a, b));
}

function lcm(...numbers) {
  let result = numbers[0];
  for (let i = 1; i < numbers.length; i++)
    result = lcmSimple(result, numbers[i]);
  return result;
}
...
module.exports = {
  ...
  maxBy, minBy, addVec, subVec, manhattanDistance, BFS, DFS, gcd, lcm
};
